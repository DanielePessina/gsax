...
def test_hand_built_design_with_ragged_expanded_layout_raises():
    """A hand-built design whose row count is not a whole number of Saltelli
    groups must be refused instead of silently dropping the trailing rows.

    ``sample()`` always produces ``n_expanded % step == 0``; only a hand-built
    ``SobolSamples`` can violate it.
    """
    sr = jaxgsa.sobol.sample(PROBLEM, n_samples=256, seed=7, verbose=False)
    step = sr.n_expanded // sr.base_n
    ragged_n = sr.n_expanded + 1
    assert ragged_n % step != 0
    samples_ragged = np.concatenate(
        [np.asarray(sr.samples)[sr.expanded_to_unique], np.asarray(sr.samples)[:1]]
    )
    hand_built = SobolSamples(
        samples=samples_ragged,
        n_expanded=ragged_n,
        expanded_to_unique=np.arange(ragged_n, dtype=np.int64),
        base_n=sr.base_n,
        n_params=sr.n_params,
        calc_second_order=sr.calc_second_order,
        problem=sr.problem,
    )
    Y_ragged = evaluate(jnp.asarray(samples_ragged))
    with pytest.raises(ValueError, match="not a multiple of the Saltelli group size"):
        jaxgsa.sobol.indices(hand_built, Y_ragged)
