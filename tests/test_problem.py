...
def _bad_params(payload: dict[str, object]) -> dict[str, InputSpecValue]:
    """Type shim: these payloads are deliberately not valid ``InputSpecValue``s."""
...
def test_from_dict_documented_extra_keys_still_parse():
    """The documented extras -- truncation bounds and level labels -- stay valid."""
    p = Problem.from_dict(
        {
            "g": GaussianInputSpec(dist="gaussian", mean=0.0, variance=1.0, low=-1.0, high=1.0),
            "c": {"dist": "categorical", "probs": [0.5, 0.5], "labels": ["a", "b"]},
        }
    )
    assert isinstance(p.input_specs[0], GaussianSpec)
    assert p.input_specs[0].low == -1.0
    assert p.categorical_labels["c"] == ("a", "b")
